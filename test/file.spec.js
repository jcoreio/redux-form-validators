...
import Validators, { file } from '../src'
...
describe('Validator: file', function () {
  ...
  it('should be valid when `value` is a File with the correct size', function () {
    ...
    Object.keys(SIZE_UNITS).forEach(function (unit) {
      ...
      assert.ok(!test(new File({ size }), { minSize: size }))
      assert.ok(!test(new FileList([{ size }]), { minSize: size }))
      assert.ok(!test(new FileList([{ size }]), { minSize: size - 1 }))
      assert.ok(!test(new FileList([{ size }]), { minSize: '1' + unit }))
      assert.ok(!test(new FileList([{ size }]), { minSize: '1 ' + unit }))
      assert.ok(!test(new FileList([{ size }]), { minSize: '1. ' + unit }))
      assert.ok(!test(new FileList([{ size }]), { minSize: '0.99 ' + unit }))
      assert.ok(!test(new FileList([{ size }]), { minSize: '0.99' + unit }))

      assert.ok(!test(new File({ size }), { maxSize: size }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: size }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: size + 1 }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: '1' + unit }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: '1 ' + unit }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: '1. ' + unit }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: '1.01 ' + unit }))
      assert.ok(!test(new FileList([{ size }]), { maxSize: '1.01' + unit }))

      assert.ok(!test(new FileList([{ size }]), { minSize: size, maxSize: size }))
      assert.ok(!test(new FileList([{ size }]), { minSize: size - 1, maxSize: size + 1 }))
    })
  })
